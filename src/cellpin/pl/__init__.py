...
class PlotAccessor:
    ...
    def losses(
        ...
        *,
        ...
        ----------
        log_path:
            Override the log directory to read from. When omitted the trainer
            output directory stored on the model is used (set automatically
            after calling :meth:`~cellpin.models.CellPin.fit`,
            :meth:`~cellpin.models.CellPin.train_model`, or
            :meth:`~cellpin.models.CellPin.pretrain_model`).
        keys:
            Which loss columns to plot, e.g. ``["val_loss", "val_reconst_loss"]``.
            Defaults to ``["val_loss", "val_reconst_loss", "val_inv_loss"]``.
            Pass ``"all"`` to show every available val loss.
        smooth:
            Centered rolling-mean window width (0 = off).
        figsize:
            ``(width, height)`` in inches. Auto-sized when omitted.
        save:
            Save the figure to this path at 300 dpi when given.
