#!/usr/bin/env python3
"""
run_cellpin_coembed.py
======================
CellPin training + co-embedding: returns AnnData with both scRNA and spatial
cells jointly embedded and imputed.

Usage
-----
uv run python run_cellpin_coembed.py \
...
def run(args: argparse.Namespace) -> None:
    ...
    st_adata = adata_sc[:, adata_sc.var_names.isin(adata_sp.var_names)]
    sp_adata = adata_sp[:, adata_sp.var_names.isin(adata_sc.var_names)]

    # ── Setup datasets for training ────────────────────────────────────────
    sc_dataset, _ = cellpin.pp.setup_data(
        sc_adata=adata_sc,
        st_adata=adata_sp,
        gene_symbols=None,
        layer=layer,
    ...
    corr_cb = CorrelationCallback(
        run_every_n_epochs=5, num_samples=1000,
        no_panel_mask=sc_dataset._no_panel_mask, verbose=True,
    )
    shared = dict(
    ...
    adata_infer = ad.concat([st_adata, sp_adata],join='outer', label="dataset", keys=["sc", "sp"])
    print("\nRunning imputation on spatial data...")
    infer_dataset, _ = cellpin.pp.setup_data(
        sc_adata=adata_infer,
        st_adata=adata_infer,
        gene_symbols=None,
        layer=layer,
    )
    dl = DataLoader(
